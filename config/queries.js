module.exports = {
    ...
    RAAGIS_SHABADS: "select rrs.id, raagi.name as raagi_name, raagi.image_url as raagi_image_url, shabad.sathaayi_title as shabad_english_title, rrs.listeners, recording.title as recording_title, shabad_info.sathaayi_id, concat('https://s3.eu-west-2.amazonaws.com/vismaadnaad/Raagis/',raagi.name, '/', shabad.sathaayi_title, '.mp3') as shabad_url, shabad_info.starting_id, shabad_info.ending_id, replace(to_char(rrs.length, 'HH24:MI:SS'), '00:','') as shabad_length, shabad_info.checked as shabad_checked, rrs.date_added from raagi_recording_shabad as rrs join shabad on rrs.shabad_id = shabad.id join shabad_info on shabad.shabad_info_id = shabad_info.id join raagi on rrs.raagi_id = raagi.id join recording on rrs.recording_id = recording.id where rrs.raagi_id=(select id from raagi where name=$1) and rrs.status='PROD' order by shabad.sathaayi_title",
    RAAGIS_RECORDINGS_SHABADS: "select rrs.id, shabad.sathaayi_title as shabad_english_title, recording.title as recording_title, shabad_info.sathaayi_id, shabad_info.starting_id, rrs.status, rrs.starting_time as shabad_starting_time, rrs.ending_time as shabad_ending_time, shabad_info.ending_id, to_char(rrs.length, 'MI:SS') as shabad_length, shabad_info.checked as shabad_checked from raagi_recording_shabad as rrs join shabad on rrs.shabad_id = shabad.id join shabad_info on shabad.shabad_info_id = shabad_info.id join recording on rrs.recording_id = recording.id where rrs.raagi_id=(select id from raagi where name = $1) and recording.title=$2 order by shabad.sathaayi_title",
    ...
    CHANGE_STARTING_TIME: "update raagi_recording_shabad set starting_time=$1 where id=$2 and starting_time=$3",
    CHANGE_ENDING_TIME: "update raagi_recording_shabad set ending_time=$1 where id=$2 and ending_time=$3",
